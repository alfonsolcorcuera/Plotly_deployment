...
function buildCharts(sample) {
  ...
  d3.json("./samples.json").then((data) => {
    ...
    var y = yticks.map(element => `OTU ${element}`).reverse();
    ...
    //11. Create the trace object for the bubble chart 
    ...
    //12. Create the layout object for the bubble chart 
    var bubbleLayout = {
    ...
    //14. Create a varible that converts the washing frequency to a floating point number
    var wfreq = parseFloat(firstMetadata.wfreq);

    //15. Create the trace object for the gauge chart 
    var gaugeData = [{
      mode: "gauge+number",
      type: "indicator",
      value: wfreq,
    ...
    //16. Create the layout object for the gauge chart 
    var gaugeLayout = { 
      title: { text: "Belly Button Washing Frequency", font: { size: 22 }},
    };

    //17. Use Plotly to plot the data with the layout. 
